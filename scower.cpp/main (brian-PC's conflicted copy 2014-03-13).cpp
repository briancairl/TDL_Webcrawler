...
void main()
{
	...
	*/
	...
	wcGlobal::init("C:\\Users\\brian\\Desktop\\math_crawl");
	
	wcGraph			graph;
	wcNode			test_node("http://en.wikipedia.org/wiki/Math");
	wcStringCell	query_vector;

	query_vector.emplace_back("math");

	test_node.perform(query_vector);
	
	test_node.dispatch( graph, query_vector );

	/*
	wcCache test_cache(test_tag);
	
	while(!test_cache.service_is_ready());

	wcPageTextAnalyzer text_pta(test_cache);
	wcPageRankAnalyzer text_pra(test_cache);

	std::cout<<text_pta.get_text_ratio()<<std::endl;

	text_pra.dump(test_cache,std::cout);
	*/
