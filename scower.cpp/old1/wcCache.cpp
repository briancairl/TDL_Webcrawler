...
wcCache::wcCache( const wcTag& tag ) :
	...
	capacity(0)
	#else
	capacity(WC_DEFAULT_BUFFER_CAP)
	#endif
{
...
{
	if((!(state_flags&wcMasks::any_failure))&&wcIS_CLEAR(state_flags,file_exists)&&(size>0))
	{
		FILE*	cache_file(fopen(tag_ref->get_cache_filename().c_str(),"w+"));
		if(cache_file)
		{
