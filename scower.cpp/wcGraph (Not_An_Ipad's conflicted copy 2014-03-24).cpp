...
{
	os<<node.tag.get_tag()		<<std::endl;
	os<<node.meta.p_quality		<<std::endl;
	os<<node.meta.p_relevence	<<std::endl;
	os<<node.meta.p_density		<<std::endl;
	os<<node.edge.size()		<<std::endl;
	for( wcEdgeList::const_iterator itr = node.edge.cbegin(); itr!= node.edge.cend(); ++itr )
	{
		os<<itr->first->tag.get_tag()	<<std::endl;
...
wcNodeStat wcNode::reassimilate( wcGraph& graph )
{
	for( wcPlaceHolderList::const_iterator plh = placeholders.cbegin(); plh != placeholders.cend(); ++plh )
	{
		...
		{
			if(static_cast<wcTag>((*node)->tag)==plh->first)
			{
...
wcGraph::wcGraph( const wcString& name ) :
	name(name)
{
